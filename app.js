...
const API_URL = "https://script.google.com/macros/s/AKfycbzLn8kPc0Abrj2MOv4kOJDFFsoOUit_vZo20qpYk17TTOha3wLso1FK7mKPl6ZiJeTJ/exec";
let session = null;
...
async function login(){
  const role=$("role").value, tps=Number($("tps").value), pin=$("pin").value.trim();
  if(!pin) return toast("Masukkan PIN");
  try{
    const d=await request(apiUrl({action:"login",role,tps,pin}));
...
function logout(){session=null;$("loginCard").hidden=false;$("tpsPanel").hidden=true;$("adminPanel").hidden=true;$("pin").value="";}
